...
class Benteng(PotonganShogi):
    def __init__(self, adalah_pemain=True):
        super().__init__("Benteng", "飛", "hi", adalah_pemain)

    def dapatkan_gerakan(self, papan, x, y):
        gerakan = []
        arah = [(0, -1), (1, 0), (0, 1), (-1, 0)]  # Atas, Kanan, Bawah, Kiri
        
        for dx, dy in arah:
            nx, ny = x + dx, y + dy
            while 0 <= nx < 9 and 0 <= ny < 9:
                if papan[ny][nx] is None:
                    gerakan.append((nx, ny))
                elif papan[ny][nx].adalah_pemain != self.adalah_pemain:
                    gerakan.append((nx, ny))
                    break
                else:
                    break
        ...
        # Gerakan tambahan untuk Naga (Benteng yang dipromosikan)
        if self.dipromosikan:
            diagonal = [(-1, -1), (1, -1), (1, 1), (-1, 1)]  # Diagonal
            for dx, dy in diagonal:
                nx, ny = x + dx, y + dy
                if 0 <= nx < 9 and 0 <= ny < 9:
                    if papan[ny][nx] is None or papan[ny][nx].adalah_pemain != self.adalah_pemain:
                        gerakan.append((nx, ny))
